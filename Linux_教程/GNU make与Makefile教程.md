...
2. 更简单的方法就是在变量引用的时候再多加一个$符号，就是说变量名前面有两个连续的$符号，例如
...
|:--|:--|
|.PHONY|先决条件被视为伪目标|
|.SUFFIXES|其先决条件被用于定义隐式规则所需的后缀|
|.DEFAULT|为所有不是显式或隐式规则的目标的先决条件设置默认命令
|.PRECIOUS|如果在命令执行过程中make被终止，其先决条件不会被删除。如果其先决条件是中介文件也不会删除
|.INTERMEDIATE|他的先决条件被视为中介文件，会自动删除，如果没有先决条件则无效
|.SECONDARY|他的先决条件被视为中介文件，但不是自动删除，如果没有先决条件则将所有的目标都是中介文件|
|.SECONDEXPANSION|将其后的所有规则的先决条件和目标项再次展开
|.DELETE_ON_ERROR|如果其先决条件的命令非0退出（出错）则删除
|.IGNORE|如果其先决条件的命令非0退出（出错）则忽略
|.LOW_RESOLUTION_TIME|由先决条件的命令创建的文件使用低精度时间戳
|.SILENT|先决条件的命令执行时不显示|
...
* 参数变量：变量由make命令行参数传入，使用`$make arg_var=value`定义
