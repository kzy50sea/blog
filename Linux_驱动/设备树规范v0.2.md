...
中断映射表中的每一行由五部分组成：**子节点单元地址**和**子节点中断说明符**，它映射到具有指定**父节点单元地址**和**父节点中断说明符**的**中断父节点**。
...
本节规定了表示Open PIC兼容中断控制器的要求。 Open PIC中断控制器实现Open PIC架构（由AMD和Cyrix联合开发），并在开放式可编程中断控制器（PIC）寄存器接口规范修订版1.2 [^b18]中指定。
...
<span id='b1'>[b4] booting-without-of.txt, Ben Herrenschmidt, Becky Bruce, et al., From the Linux kernel source tree (http://www.kernel.org/), Describes the devicetree as used by the Linux kernel.
